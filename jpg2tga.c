...
// subsampling factor).
uint8 *pjpeg_load_from_file(const char *pFilename, int *x, int *y, int *comps, pjpeg_scan_type_t *pScan_type, int reduce, FILE *coeffs)
{
    ...
    *comps = 0;
    ...
    status = pjpeg_decode_init(&image_info, pjpeg_need_bytes_callback, NULL, (unsigned char)reduce, coeffs);
    ...
    pImage = (uint8 *)malloc(row_pitch * decoded_height); //dla kolorowego full HD = 1920 * 1080 * 3 = 6220800
...
{
    FILE * coeffs1;
    coeffs1 = fopen("coefficients1.txt", "wb");
...
*/
    pSrc_filename = "street.jpg";
    pDst_filename = "street.bmp";
    printf("Source file:      \"%s\"\n", pSrc_filename);
    printf("Destination file: \"%s\"\n", pDst_filename);
    printf("Reduce during decoding: %u\n", reduce);

    pImage = pjpeg_load_from_file(pSrc_filename, &width, &height, &comps, &scan_type, reduce, coeffs1);
    if (!pImage)
    {
    ...
    */
    ...
    //Obraz nr2
    coeffs1 = fopen("coefficients2.txt", "wb");

    pSrc_filename = "street2.jpg";
    pDst_filename = "street2.bmp";
    printf("Source file:      \"%s\"\n", pSrc_filename);
    printf("Destination file: \"%s\"\n", pDst_filename);
    printf("Reduce during decoding: %u\n", reduce);
    ...
    FILE * coeffs2, * diff;
    coeffs1 = fopen("coefficients1.txt", "rb");
    coeffs2 = fopen("coefficients2.txt", "rb");
    diff = fopen("diff.txt", "wb");
    ...
    while(fread(coeffBuf1, sizeof(int16), 64, coeffs1) == 64 && fread(coeffBuf2, sizeof(int16), 64, coeffs2))
    {
        for (i = 0; i < 64; i++) {
            diffBuf[i] = coeffBuf1[i] - coeffBuf2[i];
        }

        fwrite(diffBuf, sizeof(int16), 64, diff);
    }
