...
//------------------------------------------------------------------------------
// 128 bytes
static int16 gCoeffBuf[8*8]; //zawiera 64 współczynniki każdego bloku
...
static FILE * coefficients; //plik do zapisu wspolczynnikow
//static FILE * coefficients_read;
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
unsigned char pjpeg_decode_init(pjpeg_image_info_t *pInfo, pjpeg_need_bytes_callback_t pNeed_bytes_callback, void *pCallback_data, unsigned char reduce, FILE * coeffs)
{
    uint8 status;

    coefficients = coeffs; //plik do zapisu wspolczynnikow
    //coefficients_read = fopen("coefficients1.txt", "rb");
   // coefficients_read = fopen("diff.txt", "rb");
