...
private:
    ...
    HuffmanTable* componentTablesDC[3]; // from format.h
    HuffmanTable* componentTablesAC[3];
    bool endOfFile;
    int previousDC[3];
    ...
    uchar* rawImagePointers[3];
    int lineBytes;
    unsigned int currentX[3]; // Coordinates for next block
    unsigned int currentY[3];
    int maxSample;
    bool hasSubSampling;
    int currentBlockHFactor[3], currentBlockVFactor[3];
